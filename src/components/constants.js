// import baikal from "../screen/baikal.png";
import barbershop from "../screen/barbershop.png";
import bookshop from "../screen/bookshop.png";
import stardb from "../screen/stardb.png";
import quiz from "../screen/quiz.png";
import technomart from "../screen/technomart.png";
import todo from "../screen/todo.png";
import ReStore from "../screen/ReStore.png";
import Memory from "../screen/Memory.png";
// import funbox from "../screen/funbox.jpg";
import gridTest1 from "../screen/gridTest1.jpg";
// import resume from "../screen/resume.jpg";

export const skills = [
  "HTML5",
  "CSS3",
  "Bootstrap",
  "jQuery",
  "JavaScript",
  "Adobe Photoshop",
  "Git",
  "БЭМ",
  "Google Maps Api",
  "Yandex Maps API",
  "Scss, Postcss",
  "Gulp",
  "Parcel",
  "React",
  "React-Redux",
  "React-Router"
];

export const workes = [
  {
    screen: ReStore,
    deploy: "https://short-example.herokuapp.com/",
    sourceCode:
      "https://github.com/kraken1970/Short-example-react-redux-router",
    head: "Re Store",
    head2: "SPA React",
    description:
      "Использованы: React, Redux, Router. Всего по-немногу... При загрузке в 25% случаях выводится error"
  },
  {
    screen: Memory,
    deploy: "https://memory-game-with-hooks.herokuapp.com/",
    sourceCode: "https://github.com/kraken1970/memory-game",
    head: "Memory Game",
    head2: "SPA React",
    description:
      "Использованы: React, UseState, UseEffect. Просто приятная игрушка."
  },
  {
    screen: todo,
    deploy: "https://todo-85af1.firebaseapp.com/",
    sourceCode: "https://github.com/kraken1970/New_Todo-react",
    head: "Todo List React",
    head2: "SPA React",
    description: "Просто todo на react"
  },
  {
    screen: quiz,
    deploy: "https://quiz-react-95dda.firebaseapp.com/",
    sourceCode: "https://github.com/kraken1970/Quiz-react",
    head: "Quiz React",
    head2: "SPA React",
    description:
      "Использованы: React, Redux, Router. Deploy and database ih firebase"
  },
  {
    screen: bookshop,
    deploy: "https://books-shop-react-2f877.firebaseapp.com/",
    sourceCode: "https://github.com/kraken1970/books-shop-react",
    head: "Books Shop React",
    head2: "SPA React",
    description:
      "Использованы: React, Redux, Router, redux-logger, redux-thunk, lodash, axios"
  },
  {
    screen: stardb,
    deploy: "https://starbook-a0b8c.firebaseapp.com/",
    sourceCode: "https://github.com/kraken1970/Star-Book",
    head: "Star Wars DB React",
    head2: "SPA React",
    description: "Использованы: React,  Router. Всего по-немногу..."
  },
  {
    screen: barbershop,
    deploy: "https://barber-shop-2f8b7.firebaseapp.com/",
    sourceCode: "https://github.com/kraken1970/BarberShop",
    head: "BarberShop",
    description:
      "Просто верстка 4 страницы + modal form  + modal 'как нас найти'"
  },
  {
    screen: gridTest1,
    deploy: "https://training-grid-e0997.firebaseapp.com/",
    sourceCode: "https://github.com/kraken1970/Training-grid",
    head: "Grid Layout test",
    description: "Просто response верстка Grid layout"
  }
  // {
  //   screen: technomart,
  //   deploy: "https://technomarket-e64f1.firebaseapp.com/",
  //   sourceCode: "https://github.com/kraken1970/Technomart",
  //   head: "Techno Market"
  // }
  // {
  //   screen: baikal,
  //   deploy: "https://baical-deb6d.firebaseapp.com/",
  //   sourceCode: "https://github.com/kraken1970/Baikal-I-Shop",
  //   head: "I-Shop Baical"
  // }
];
